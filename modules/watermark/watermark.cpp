...
void RenderWatermark(bool menuOpen)
{
    ...
    float x_pos = g_WatermarkSettings.posX;
    float y_pos = g_WatermarkSettings.posY;
    
    // Handle dragging (only when menu is open)
    if (menuOpen)
    {
        if (ImGui::IsMouseClicked(0))
        {
            // Check if clicking on watermark
            float text1_width = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, 0.0f, "cubixDLC").x;
            float text2_width = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, 0.0f, "| 0.1v | dll | 9999 FPS").x;
            float total_content_width = text1_width + 5.0f + text2_width;
            float actual_width = total_content_width + padding * 2;
            
            if (mousePos.x >= x_pos && mousePos.x <= x_pos + actual_width &&
                mousePos.y >= y_pos && mousePos.y <= y_pos + height)
            {
                g_WatermarkSettings.isDragging = true;
                g_WatermarkSettings.dragOffsetX = mousePos.x - x_pos;
                g_WatermarkSettings.dragOffsetY = mousePos.y - y_pos;
            }
        }
        
        static float lastWatermarkSaveTime = 0.0f;
        
        if (!isMouseDown)
        {
            if (g_WatermarkSettings.isDragging)
            {
                // Save immediately when dragging stops
                SaveCategoriesToConfig();
                lastWatermarkSaveTime = (float)ImGui::GetTime();
            }
            g_WatermarkSettings.isDragging = false;
        }
        
        if (g_WatermarkSettings.isDragging && isMouseDown)
        {
            x_pos = mousePos.x - g_WatermarkSettings.dragOffsetX;
            y_pos = mousePos.y - g_WatermarkSettings.dragOffsetY;
            ...
            x_pos = std::max(0.0f, std::min(screenSize.x - actual_width, x_pos));
            y_pos = std::max(0.0f, std::min(screenSize.y - height, y_pos));
            
            g_WatermarkSettings.posX = x_pos;
            g_WatermarkSettings.posY = y_pos;
            
            // Auto-save while dragging (with delay)
            float currentTime = (float)ImGui::GetTime();
            if (currentTime - lastWatermarkSaveTime > 0.5f) // Save every 0.5 seconds while dragging
            {
                SaveCategoriesToConfig();
                lastWatermarkSaveTime = currentTime;
            }
        }
    }
